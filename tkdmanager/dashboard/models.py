...
class Style(models.Model):
    ...
    def get_default_pk(cls):
        style, created = cls.objects.get_or_create(name="TKD")
        return style.pk


class Belt(models.Model):
    style = models.ForeignKey(
        Style, on_delete=models.PROTECT, default=Style.get_default_pk
    )
...
class Member(models.Model):
    ...
    address_line_1 = models.CharField(
        max_length=200, help_text="Unit, Street Number and Name", blank=True
    )
    ...
    belts = models.ManyToManyField(
        Belt, default=Belt.get_default_pk, related_name="belts"
    )
    email = models.EmailField()
    phone = models.CharField(max_length=100)
    team_leader_instructor = models.CharField(
        max_length=2,
        choices=TL_INST_RANKS,
        blank=True,
        verbose_name="Team Leader/Instructor",
    )
    active = models.BooleanField(default=True)
    properties = models.ManyToManyField(
        "MemberProperty", related_name="properties", blank=True
    )
    ...
    def get_class_types_pretty(self):
        ...
        class_types = set(
            ...
            for obj in self.students2classes.filter(date__gte=six_months_ago)
        )
        return class_types
    ...
    def save(self, *args, **kwargs):
        ...
        if has_duplicate_styles(belts):
            ...
            raise ValidationError("A member can't have two belts from the same style!")
    ...
    def belt(self):
        ...
        """
        raise NoLongerWorks(
            "You can't just read a belt from a member - specify a style and use get_belt(style)"
        )
...
class AssessmentUnit(models.Model):
    ...
    grading_result = models.ForeignKey(
        ...
        verbose_name="Associated Grading Result",
    )
...
class GradingResult(models.Model):
    member = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name="member2gradings"
    )
    assessor = models.ManyToManyField(
        Member,
        help_text="Who assessed this particular grading?",
        related_name="assessor2gradings",
    )
    forbelt = models.ForeignKey(
        Belt,
        on_delete=models.PROTECT,
        default=Belt.get_default_pk,
        verbose_name="For Belt",
    )
    comments = models.CharField(max_length=300, blank=True)
    award = models.ForeignKey(
        Award, on_delete=models.RESTRICT, verbose_name="Award", null=True, blank=True
    )
    is_letter = models.BooleanField(default=False)
    gradinginvite = models.OneToOneField(
        "GradingInvite",
        on_delete=models.CASCADE,
        null=True,
        blank=True,
        verbose_name="Grading Invite",
    )
    grading = models.ForeignKey(Grading, on_delete=models.SET_NULL, null=True)

    class Meta:
        ordering = [
            "-grading__grading_datetime",
            "-forbelt",
            "grading__grading_type",
            "member__idnumber",
        ]
...
class GradingInvite(models.Model):
    ...
    def __str__(self):
        if self.grading:
            ...
            return (
                f"GRADINGINVITE_NULL_GRADING for {self.forbelt.name}, by {self.member}"
            )
...
class Class(models.Model):
    ...
    instructors = models.ManyToManyField(
        Member,
        help_text="Who taught this class?",
        related_name="instructors2classes",
        blank=True,
    )
    students = models.ManyToManyField(
        Member,
        help_text="Who attended this class?",
        related_name="students2classes",
        blank=True,
    )
...
class Payment(models.Model):
    member = models.ForeignKey(
        Member, help_text="Who needs to pay this?", on_delete=models.PROTECT
    )
    paymenttype = models.ForeignKey(
        "PaymentType",
        help_text="What type of payment is this?",
        null=True,
    ...
    amount_due = models.DecimalField(
        max_digits=7, decimal_places=2, help_text="Amount to be paid, in $", default=0
    )
    amount_paid = models.DecimalField(
        max_digits=7,
        decimal_places=2,
        help_text="Amount currently paid, in $",
        default=0,
    )
...
class PaymentType(models.Model):
    ...
    standard_amount = models.DecimalField(
        max_digits=7,
        decimal_places=2,
        help_text="Standard amount to be paid, in $",
        default=0,
    )
...
class RecurringPayment(models.Model):
    ...
    payments = models.ManyToManyField(
        Payment, help_text="What payments are linked to this", blank=True
    )
    last_payment_date = models.DateField(
        null=True
    )  # should be the same as the creation date of the most recent payment
    interval = models.DurationField(default=timedelta(days=30), help_text="DD HH:MM:SS")
    amount = models.DecimalField(
        max_digits=7, decimal_places=2, help_text="Amount to be paid, in $", default=0
    )
    next_due = models.GeneratedField(
        db_persist=True,
        output_field=models.DateTimeField(),
        expression=(F("last_payment_date") + F("interval")),
    )
    paymenttype = models.ForeignKey(
        PaymentType, verbose_name="Payment Type", on_delete=models.PROTECT
    )
    ...
    def __str__(self):
        return (
            f"Recurring Payment for {self.member}, ${self.amount} per {self.interval}"
        )
...
class MemberProperty(models.Model):
    ...
    propertytype = models.ForeignKey(
        "MemberPropertyType", on_delete=models.CASCADE, verbose_name="Property Type"
    )
    member = models.ManyToManyField(
        Member, through=Member.properties.through, related_name="members", blank=True
    )
    name = models.CharField(max_length=200)
